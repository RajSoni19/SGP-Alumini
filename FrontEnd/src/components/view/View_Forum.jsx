...
const ViewTopic = () => {
    ...
    useEffect(() => {
        if (topic) {
            axios.post(`${baseUrl}auth/topiccomments`, { topic_id: topic.id })
                .then(response => {
    ...
    const handleSave = async (id) => {
        try {
            const response = await axios.put(`${baseUrl}auth/view_forum/${id}`, {
                comment: newComment
            });
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`${baseUrl}auth/view_forum/${id}`);
    ...
    const handleCommentSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`${baseUrl}auth/view_forum`, {
                c: newComment,
                user_id: userid,
                topic_id: topic.id
            });
                            ...
                            {comments.map((c, index) => (
                                <div className="form-group comment" key={index}>
                                    {editingCommentId === c.id ? (
                                        ...
                                        <>
                                            {isLoggedIn && c.user_id == localStorage.getItem("user_id") && (
